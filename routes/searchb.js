...
router.post('/',async function(req,res,next){
	...
	result = await con.q('SELECT * FROM lar_data WHERE start BETWEEN ? AND ? ORDER BY start ASC',[sTime,eTime])
	result2 = await con.q('SELECT userName,swtime,ewtime FROM privacy_data')
    log.logger('info','Admin List Leave Report : '+ req.cookies.user_name)
    var setime = {}
	...
	for (var i = 0; i < result.length; i++) {
			...
			timeKeep = ll.getDayTime(result[i].start,(end?end:end),allDay,setime[result[i].userName][0],setime[result[i].userName][2])
