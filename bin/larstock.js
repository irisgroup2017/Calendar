 ...
 */
 ...
 if (!ov[0]) {
  if (w < 1) {
   ...
   vap = '0' + Math.floor((12 - (x[1] + 1) / 2)) + '0000'
  } else if (w > 1) {
   vap = '060000'
  }
 } else {
  ...
  if (ovr || ovr == 0) {
   ...
   if (state == 'insert') {
    ...
    await con.q('UPDATE lar_status SET userName = ?,vacationp = ? WHERE dataid = ? AND year = ?', [userName, ovr, dataid, y[2]])
   }
  } else if (ovp || ovp == 0) {
   ...
   if (state == 'insert') {
    ...
    await con.q('UPDATE lar_status SET userName = ?,vacationp = ? WHERE dataid = ? AND year = ?', [userName, ovp, dataid, y[2]])
   }
  } else if (ovq || ovq == 0) {
   if (state == 'insert') {
    vap = ovq
   } else {
    await con.q('UPDATE lar_status SET userName = ?,vacationp = ? WHERE dataid = ? AND year = ?', [userName, ovq, dataid, y[2]])
   }
  } else {
   if (ov[0].vacation || ov[0].vacation == 0) {
...
async function updateEnt(dataid) {
 ...
 for (var i = firstYear; i <= thisYear; i++) {
  ...
  await larlist.viewLar(name, dataid, date)
 }
}
...
async function updateLar(userName, dataid, now) {
 ...
 if (checkdate == '') {
  await setLar(userName, dataid, 'insert', now)
 } else {
  await setLar(userName, dataid, 'update', now)
 }
}
